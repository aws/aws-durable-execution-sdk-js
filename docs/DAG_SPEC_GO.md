...
**Concurrency defaults.** `DefaultDagMaxConcurrency` is `40`. When `WithDagMaxConcurrency` is unset, the DAG runs at most 40 top-level tasks concurrently — **not** unbounded, unlike `Map`/`Parallel`, which default to unbounded when their own concurrency option is omitted. `WithDagMaxConcurrency(n)` with a negative `n` is a configuration error (`*DagInvalidConfigError`), rejected before the scope is entered. Pass `0` to opt back into unbounded top-level concurrency — this is the one case where `0` is not a no-op: it explicitly removes the default-40 cap rather than leaving it unset. An explicit value always wins, including a value above 40. The bound governs only this DAG's top-level tasks: a Map/Parallel task's inner fan-out is bounded separately by `WithBatchMaxConcurrency`, and a nested `SubDag` resolves its own independent default of 40 unless it sets its own bound (because `SubDag` runs its nested graph back through `Dag`).
...
- `WithDagMaxConcurrency(n)` with a negative `n` ⇒ `Dag(...)` returns `*DagInvalidConfigError`, evaluated before the scope is entered. `n == 0` is valid and means unbounded.
...
- **`dag_option_validation_test.go`**: inapplicable-option rejection per operation kind; negative `WithDagMaxConcurrency`; mutually-exclusive completion config; required `WithCondition`.
