...
| 15  | Custom result-based completion (§13.4)                     | **Port** (origin)                  | **Ported** (added post-v1: `DagCustomCompletionConfig`/`should_complete`, no base-SDK hook needed — built fresh for DAG) | **Ported** (added post-v1: `DagCompletionConfig.custom(...)`/`CustomDagCompletion`, compile-time-exclusive with threshold config via sealed interface) | **Port** (origin-parity: `ShouldComplete` predicate, shipped from the start alongside TS)                                                                            |
...
2. **Custom result-based completion (§13.4) — resolved, ported to all four SDKs.** TS and Go shipped it from v1. Python and Java did not initially — both carried an in-source note calling the gap "v2-deferred," which on inspection referred to the _base SDK's_ `map`/`parallel` completion feature, not DAG; DAG itself needed no such deferral and both languages have since added `DagCustomCompletionConfig`/`DagCompletionConfig.custom(...)` directly on the DAG module, without waiting on or depending on a base-SDK predicate hook for map/parallel. Java's version is compile-time-exclusive with threshold config (the two are separate sealed-interface records, not one class with nullable fields, which TS/Go's structural typing only enforces at the type-checker level). Python's was built without any base-SDK precedent to lean on at all (Python's `CompletionConfig` for map/parallel is still purely threshold-based). _Outcome:_ the earlier recommendation to "introduce a base predicate hook for map/parallel + dag jointly to avoid drift" was not followed — DAG shipped its own independent predicate mechanism per language instead, which is now the norm across all four SDKs for DAG specifically. Whether map/parallel should retrofit the same predicate hook (where absent) remains open and unrelated to DAG's completeness.
