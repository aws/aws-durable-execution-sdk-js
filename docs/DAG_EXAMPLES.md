...
**`runIf` is a gate on data, evaluated by the scheduler.** It receives the same typed `deps` map the body would, and returns a boolean. `false` means the task is skipped with `skipReason: "RUN_IF_PREDICATE"` — again, no events, no cost. The predicate must be **synchronous and deterministic**: no `async`, no IO, no clock. It is re-evaluated on replay and has to reach the same verdict, and it is not a checkpointed operation, so anything it does is not durable. Because it is pure, it should never fail on the data itself: if the predicate **throws**, that is a defect, and the DAG aborts — the task gets no terminal state and `dag(...)` rejects with a `DagPredicateError` (cause = your error). It is not silently turned into a task `FAILED`, which would misroute every downstream failure-compensation branch.
...
**Async or side-effecting `runIf`.** The predicate is synchronous, deterministic, and not a checkpointed operation. It is re-evaluated on replay and must reach the same verdict. Anything it does — a log, a counter, an API call — is not durable and may happen a different number of times than you expect. And because a pure predicate has no business reason to fail, a `runIf` that **throws** is treated as a defect, not an outcome: the task gets no terminal state (neither `FAILED` nor `SKIPPED`), the scheduler starts no further tasks, and the whole `dag(...)` operation rejects with a `DagPredicateError` whose `cause` is your original error and whose message names the offending task. It deliberately does **not** become a task `FAILED` — otherwise a `TypeError` in a predicate would fire every downstream `ALL_FAILED` / `ANY_FAILED` / `ALL_DONE` compensation (e.g. issue a refund), letting a scheduler-side defect masquerade as a real upstream outcome. Keep predicates total; a throwing body is a normal `FAILED`, but a throwing predicate aborts.
