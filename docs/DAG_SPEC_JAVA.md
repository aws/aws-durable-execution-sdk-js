...
Semantics match JS: a default drain distinguishes clean (`ALL_COMPLETED`) from drained-with-failures (`COMPLETED_WITH_FAILURES`), so the reason itself disambiguates and `throwIfError()` keys off `failureCount()` — and also off `completionReason() == CUSTOM_COMPLETION_FAILED`, since a custom predicate can fail the DAG with zero individual task failures. `CUSTOM_COMPLETION_SUCCEEDED`/`CUSTOM_COMPLETION_FAILED` are defined alongside the other four members. The string values of the shared members are identical across SDKs, which is what keeps checkpoints diagnosable cross-language (cross-language doc §2.A.3).
...
**Custom-predicate completion is implemented.** The base SDK's own completion surface has no custom-predicate hook and no `CUSTOM_COMPLETION_*` status (`ConcurrencyCompletionStatus` is closed at three members; completion is factory-method-only), so result-based short-circuit (JS §13.4) is a net-new, DAG-owned feature — it does not extend or depend on the base batch layer. `DagCompletionConfig` is sealed over two implementations: `ThresholdDagCompletion` (the six factory methods) and `CustomDagCompletion` (`DagCompletionConfig.custom(Function<DagCompletionStatus, DagCompletionDecision>)`). Because the two are separate sealed-interface records rather than one class with nullable fields, mutual exclusivity between threshold and custom configuration is a compile-time impossibility, not a runtime check — stronger than the structural-typing guarantee TS/Go rely on. `DagExecutor` evaluates the custom predicate after every task settlement with a live `DagCompletionStatus` snapshot (per-task status and result, in registration order); a `complete` decision ends the DAG with `CUSTOM_COMPLETION_SUCCEEDED` or `CUSTOM_COMPLETION_FAILED` depending on the decision's outcome.
...
| e   | Completion-reason core/superset layering                                 | **Adapts**                 | Java cannot union enums; a DAG-local `DagCompletionReason` (§2.10) is a superset of the 3-member `ConcurrencyCompletionStatus` plus `COMPLETED_WITH_FAILURES`, `CUSTOM_COMPLETION_SUCCEEDED`, and `CUSTOM_COMPLETION_FAILED`.       |
| e   | Custom completion predicate w/ result-based short-circuit                | **Ships (net-new)**        | Base batch has no custom-predicate hook / `CUSTOM_COMPLETION_*`; `DagCompletionConfig.custom(...)` is a DAG-owned addition, sealed-interface-exclusive with threshold config, evaluated by `DagExecutor` after every settlement.    |
