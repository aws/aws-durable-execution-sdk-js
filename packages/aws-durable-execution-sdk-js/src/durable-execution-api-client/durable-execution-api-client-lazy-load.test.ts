import { loadLambdaModule, resolveDefaultLambdaClient } from "./lambda-module";
 ...
 * command constructors — a real risk, because a dynamic import from CommonJS can place a
 * module's exports under `default` instead of as named exports. This file deliberately does
 * **not** mock the module, so it exercises the real resolution path.
 */
describe("lazy Lambda module resolution", () => {
  ...
  it("memoizes the load so concurrent callers share one import", async () => {
  ...
  it("constructs a usable default client without issuing a request", async () => {
    const client = await resolveDefaultLambdaClient();

    expect(typeof client.send).toBe("function");
  });

  it("caches the default client so connections are reused", async () => {
    const [first, second] = await Promise.all([
