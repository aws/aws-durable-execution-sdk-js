...
describe("Promise Handler", () => {
  ...
  describe("retry behavior", () => {
    it("should use runInChildContext without retry configuration", async () => {
      const promises = [
        new DurablePromise(() => Promise.resolve(1)),
        new DurablePromise(() => Promise.resolve(2)),
      ];
      mockRunInChildContext.mockResolvedValue([1, 2]);

      await promiseHandler.all(promises);

      // runInChildContext is called with name, function, and config with errorClass
    ...
    it("should accept undefined as name parameter", async () => {
      ...
      mockRunInChildContext.mockResolvedValue([1]);
    ...
    it("should pass name to runInChildContext when provided", async () => {
      const promises = [
        new DurablePromise(() => Promise.resolve(1)),
        new DurablePromise(() => Promise.resolve(2)),
      ];
