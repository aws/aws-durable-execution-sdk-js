...
class Article {
  ...
  isPublished(): boolean {
    // Calls a Date method rather than checking `instanceof`, so this only works
    // if publishedAt was rehydrated into a usable Date and not left as the ISO
    // string JSON serialization produced.
    return this.metadata.publishedAt.getTime() > 0;
  }
}
