import { handler } from "./otel-wait-for-condition";
...
createTests({
  ...
  tests: (runner, { assertEventSignatures }) => {
    ...
    it("normal mode - condition met after 3 polls", async () => {
      ...
      // At least 1 STEP span visible in the final invocation's exported spans
      // (exporter.reset() is called at each invocation start, so only the last
      // invocation's spans are captured; earlier polling spans are lost)
      const stepSpans = spans.filter(
        (s) =>
          s.attributes["durable.operation.type"] === "STEP" &&
          s.attributes["durable.operation.attempt"] === undefined,
