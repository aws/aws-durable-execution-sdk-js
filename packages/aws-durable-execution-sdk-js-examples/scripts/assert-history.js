...
if (!fs.existsSync(expectedHistoryPath)) {
  console.log(
    `No expected history file found at ${expectedHistoryPath}, skipping assertion`,
  );
  process.exit(0);
}
...
for (const [signature, expectedCount] of expectedCounts) {
  ...
  if (actualCount !== expectedCount) {
    ...
    console.error(
      `Event signature mismatch for ${eventInfo.EventType}${eventInfo.SubType ? `(${eventInfo.SubType})` : ""}${eventInfo.Name ? ` "${eventInfo.Name}"` : ""}: expected ${expectedCount}, got ${actualCount}`,
    );
    process.exit(1);
  }
}
...
for (const [signature, actualCount] of actualCounts) {
  if (!expectedCounts.has(signature)) {
    const eventInfo = JSON.parse(signature);
    console.error(
      `Unexpected event: ${eventInfo.EventType}${eventInfo.SubType ? `(${eventInfo.SubType})` : ""}${eventInfo.Name ? ` "${eventInfo.Name}"` : ""} (${actualCount} occurrences)`,
    );
    process.exit(1);
  }
}
