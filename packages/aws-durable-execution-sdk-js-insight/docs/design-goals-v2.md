...
#### 2. Customer-owned storage (no platform-internal data store)
...
#### 3. No new query API in our service
...
#### 6. Fully configurable data content
...
#### 8. Support both in-progress and finished executions
...
#### 9. Processed (curated) data, not raw events
...
#### 14. First-class AWS service destinations
...
Each destination has different trade-offs (cost, query capability, latency, update support) and serves different customer patterns.

#### 15. Direct third-party destination support (no double-hop)
...
The plugin uses an internal queue to batch emissions efficiently. In in-progress mode, each status change produces a new version of the full curated record and enqueues it. Since each record represents the complete current state of the execution (not a delta), newer records supersede older ones — if multiple status changes occur before the queue is flushed, intermediate records are dropped and only the latest is sent. This means 100 rapid status changes result in far fewer than 100 destination writes. The queue flushes in batches at configurable intervals or when the execution suspends/completes, whichever comes first. This reduces destination cost, network calls, and compute overhead while ensuring the destination always receives the most up-to-date state.
...
#### 20. Schema stability & forward compatibility
...
#### 24. Configuration changes apply to new executions only
...
#### 26. Retention decoupled from execution retention
...
#### 29. Testability in local development
...
#### 30. Workflow Insight UI with natural language query
...
- **Third-party / custom destinations:** The Workflow Insight UI cannot query arbitrary external systems out of the box. For these, customers use the open-source customizable UI library (Goal 31) and implement a query adapter that defines how to send queries and receive results from their destination.

#### 31. Multiple access surfaces (Console, IDE, customizable open-source UI)
...
#### 32. Backfill historical executions _(post-initial launch)_
